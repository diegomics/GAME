...
game_download_with_timeout() {
    ...
    echo "[GAME] Running with ${STALL_TIMEOUT}s timeout: $@"
...
# ---------------------------------------------------------------------
game_http_fetch_stdout() {
    # game_http_fetch_stdout <url>
    local URL=$1
...
req = urllib.request.Request(sys.argv[1], headers={"User-Agent": "GAME/urllib"})
sys.stdout.write(urllib.request.urlopen(req, timeout=60).read().decode())
' "$URL"
    else
        echo "[GAME] No HTTP client available in container" >&2
        return 127
    fi
}
...
# ---------------------------------------------------------------------
game_http_fetch() {
    # game_http_fetch <url> <output_file>
...
for attempt in range(1, 6):
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "GAME/urllib"})
        with urllib.request.urlopen(req, timeout=14400) as r, open(out, "wb") as f:
            shutil.copyfileobj(r, f, length=1024 * 1024)
        sys.exit(0)
    except (urllib.error.URLError, OSError) as e:
        sys.stderr.write(f"[GAME]   python urllib attempt {attempt} failed: {e}\n")
        if attempt < 5:
            time.sleep(60)
sys.exit(1)
PYEOF
    else
        echo "[GAME] No HTTP client (curl/wget/python) available in container" >&2
        return 127
    fi
}
...
game_ena_get_urls() {
    ...
    echo "[GAME] Querying ENA portal API for ${ACC} (mode=${MODE})..." >&2

    META=$(game_http_fetch_stdout \
        "${API_URL}?accession=${ACC}&result=read_run&fields=${FIELDS}&format=tsv")
    FETCH_EXIT=$?
    if [ "$FETCH_EXIT" -ne 0 ]; then
        echo "[GAME] ENA portal API query failed (exit ${FETCH_EXIT})" >&2
        return 2
    fi

    ROW=$(echo "$META" | awk 'NR==2')
    if [ -z "$ROW" ]; then
        echo "[GAME] ENA portal returned no data row for ${ACC}" >&2
        echo "[GAME] Response was:" >&2
        echo "$META" >&2
        return 3
    fi
    ...
    if [ -n "$FASTQ_FTP" ]; then
        ...
        echo "[GAME] No FASTQ available via ENA portal for ${ACC}" >&2
        echo "[GAME]   fastq_ftp='${FASTQ_FTP}'" >&2
        echo "[GAME]   submitted_ftp='${SUBMITTED_FTP}'" >&2
        echo "[GAME]   submitted_format='${SUBMITTED_FORMAT}'" >&2
        return 4
    fi

    COUNT=$(echo "$URLS" | tr ';' '\n' | grep -c .)
    if [ "$MODE" = "paired" ] && [ "$COUNT" -ne 2 ]; then
        echo "[GAME] Expected 2 URLs for paired-end ${ACC}, got ${COUNT}" >&2
        return 5
    fi
    if [ "$MODE" = "single" ] && [ "$COUNT" -lt 1 ]; then
        echo "[GAME] No URLs for ${ACC}" >&2
        return 5
    fi

    echo "[GAME] Found ${COUNT} URL(s) for ${ACC} via ${SOURCE}" >&2
...
# ---------------------------------------------------------------------
game_ena_download_paired() {
    local ACC=$1
    local URLS URL1 URL2 API_EXIT

    URLS=$(game_ena_get_urls "$ACC" paired)
    API_EXIT=$?
    ...
    game_http_fetch "$URL1" "${ACC}_1.fastq.gz" || return 10
    game_http_fetch "$URL2" "${ACC}_2.fastq.gz" || return 11
    return 0
}
...
# ---------------------------------------------------------------------
game_ena_download_single() {
    local ACC=$1
    local URLS URL1 API_EXIT

    URLS=$(game_ena_get_urls "$ACC" single)
    API_EXIT=$?
    if [ "$API_EXIT" -ne 0 ]; then
        return "$API_EXIT"
    fi
    ...
    URL1=$(echo "$URLS" | sed -n '1p')
    game_http_fetch "$URL1" "${ACC}.fastq.gz" || return 10
    return 0
}
