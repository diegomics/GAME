...
class ResourceCalculator:
    ...
    def calculate_runtime(self, rule_name: str, input_files: List = None, attempt: int = 1) -> int:
            ...
            if runtime > max_runtime:
                print(f"[ResourceCalc] {rule_name}: calculated {runtime:.0f} min, capping to {max_runtime:.0f} min (myRUN={self.user_runtime}h)", file=sys.stderr)
            
            runtime = min(runtime, max_runtime)
            
            # Safety check - ensure we return a valid positive integer
            if runtime <= 0 or runtime != runtime:  # catches NaN
                print(f"[ResourceCalc] WARNING: Invalid runtime for {rule_name}: {runtime}, defaulting to 60 min", file=sys.stderr)
                runtime = 60
